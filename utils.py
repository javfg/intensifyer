...
def check_image_restrictions(update):
    ...
    logger.info(f"[{user_data(update)}] checking image {image.file_id}"
                + f"({image.width}x{image.height}, {image.file_size})")

    if (image.width > config.MAX_WIDTH
            or image.height > config.MAX_HEIGHT
            or image.file_size > config.MAX_FILE_SIZE):
...
def check_sticker_restrictions(update):
    ...
    logger.info(f"[{user_data(update)}] checking sticker {sticker.file_id}"
